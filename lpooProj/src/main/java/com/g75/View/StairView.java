...
import com.g75.Model.StairModel;
...
public class StairView {
    ...
    public void draw(TextGraphics graphics, StairModel stairModel) {
        if (stairModel.getIsActive() == 1) {
            for (int i = 0; i < sprite.width; i++) {
                for (int j = 0; j < sprite.height; j++) {
                    ...
                    if (sprite.unicodeChar[i][j] != null && sprite.background[i][j] != null)
                        graphics.putString(stairModel.getPosition().x + i, stairModel.getPosition().y + j, sprite.unicodeChar[i][j]);
                }
            }
        }
    }
